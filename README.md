...
`1.1.1` adds recovery for Chrome Prompt API sessions that expire or are destroyed during page scans. `1.1.0` added periodic spoiler memory, labeled image examples, safer VLM image loading, a redesigned redaction-bureau UI, and deterministic adversarial/wild test harnesses. Chrome Web Store publication requires creating store listing assets and completing Google's developer account workflow.
